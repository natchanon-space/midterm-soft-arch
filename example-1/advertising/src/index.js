...
function setupHandlers(microservice) {
    ...
    microservice.app.get("/advertising", (req, res) => {
        return adsCollection.aggregate([{ $sample: { size: 1 } }])
            ...
            .catch(err => {
                console.error("Failed to get ads from database!");
    ...
    //     return advertisingIdCollection.aggregate([{ $sample: { size: 1 } }]) // Returns a promise so we can await the result in the test.
    //         .then(advertisingId => {
    //             if (!advertisingId) {
